...
{
    renderBackend->DrawLine(0, x1, y1, x2, y2, color, alpha, inkEffect, screenRelative);
}
...
{
    renderBackend->DrawRectangle(0, x, y, width, height, color, 
                                 alpha, inkEffect, screenRelative);
}

void RSDK::DrawCircle(int32 x, int32 y, int32 radius, uint32 color, int32 alpha, int32 inkEffect, bool32 screenRelative)
{
    // TODO: proper drawGroup implementation
    renderBackend->DrawCircle(0, x, y, radius, color, alpha, inkEffect, screenRelative);
}
...
{
    renderBackend->DrawFace(0, vertices, vertCount, r, g, b, alpha, inkEffect);
}

void RSDK::DrawBlendedFace(Vector2 *vertices, uint32 *colors, int32 vertCount, int32 alpha, int32 inkEffect)
{
    renderBackend->DrawBlendedFace(0, vertices, colors, vertCount, alpha, inkEffect);
}
...
void RSDK::DrawSpriteFlipped(int32 x, int32 y, int32 width, int32 height, int32 sprX, int32 sprY, int32 direction, int32 inkEffect, int32 alpha,
                             int32 sheetID)
{
    renderBackend->DrawSpriteFlipped(0, x, y, width, height, sprX, sprY, direction,
...
{
    renderBackend->DrawSpriteRotozoom(0, x, y, pivotX, pivotY, width, height, sprX, sprY, 
                                      scaleX, scaleY, direction, rotation, inkEffect,
                                      alpha, sheetID);
}

void RSDK::DrawDeformedSprite(uint16 sheetID, int32 inkEffect, int32 alpha)
{
    renderBackend->DrawDeformedSprite(0, sheetID, inkEffect, alpha);
}

void RSDK::DrawTile(uint16 *tiles, int32 countX, int32 countY, Vector2 *position, Vector2 *offset, bool32 screenRelative)
{
    renderBackend->DrawTile(0, tiles, countX, countY, position, offset, screenRelative);
}

void RSDK::DrawAniTile(uint16 sheetID, uint16 tileIndex, uint16 srcX, uint16 srcY, uint16 width, uint16 height)
{
    renderBackend->DrawAniTile(0, sheetID, tileIndex, srcX, srcY, width, height);
}
